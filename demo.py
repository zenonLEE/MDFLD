...
if __name__ == '__main__':
    ...
    data_path = 'G:\FFHQ/'
    main_save_path = 'G:\FFHQ_lm/'
    os.makedirs(main_save_path, exist_ok=True)
    img_list = glob(data_path+'/*/*.png')

    false_c = 0
    for i in range(len(img_list)):

        save_dir = main_save_path + img_list[i].split('\\')[2] + '/'
        os.makedirs(save_dir, exist_ok=True)
        save_file_name = save_dir + img_list[i].split('\\')[3]
        if os.path.exists(save_file_name) is False:
            ...
            if empty is not None:
                ...
                cv2.imwrite(save_file_name, empty)
            else:
                false_c+=1
    print('total false count:', false_c)
